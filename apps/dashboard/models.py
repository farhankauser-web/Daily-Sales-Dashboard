...
# ── P0.2 FBA Fee Intelligence Phase 2 ───────────────────────────────────────

class FbaFeeComponent(models.Model):
    """
    Per-day per-MSKU per-component FBA fee breakdown from Data Kiosk.
    Stores atomic fee component data: base fulfillment, fuel surcharge, etc.
    """
    ...
    component_name = models.CharField(
        ...
        help_text='FeeComponent.name (free text): BaseFbaFulfilmentFee, FuelSurcharge, LowInventoryLevelFee, etc.'
    )
    quantity = models.FloatField(
        default=0.0,
        help_text='Billed units for this component. May be 0.0 or null on adjustment rows (reclassification between components).'
    )
    ...
    amount_per_unit = models.DecimalField(
        ...
        help_text='Final per-unit charge AFTER promotion and tax. Never use for packaging analysis per Section J.'
    )
    ...
    is_amzn_generated = models.BooleanField(
        db_index=True, default=False,
        help_text='K1: msku begins with amzn. (grade-and-resell). Excluded from all impact analysis per business rule.'
    )
    is_adjustment = models.BooleanField(
        default=False,
        help_text='K2: quantity=0 or amount<0, indicating reclassification between components. Stored but excluded from rate derivation.'
    )
    query_date_range_start = models.DateField(
        blank=True, null=True,
        help_text='startDate of the Data Kiosk query this row came from.'
    )
    query_date_range_end = models.DateField(
        blank=True, null=True,
    ...
    class Meta:
        ...
        indexes = [
            models.Index(fields=['marketplace', '-date'], name='ix_fbafeecomponent_mp_date'),
            models.Index(fields=['marketplace', 'msku', '-date'], name='ix_fbafeecomponent_mp_sku_date'),
            models.Index(fields=['is_amzn_generated', '-date'], name='ix_fbafeecomponent_amzn_date'),
            models.Index(fields=['is_adjustment', '-date'], name='ix_fbafeecomponent_adj_date'),
        ]
        constraints = [
            models.UniqueConstraint(
                fields=['marketplace', 'msku', 'date', 'fee_type', 'component_name'],
                name='ix_fbafeecomponent_unique'
            ),
        ]

    def __str__(self):
        return f'{self.marketplace.upper()} {self.msku} {self.date} [{self.component_name}]'


class DataKioskIngestLog(models.Model):
    """
    Audit trail for each Data Kiosk ingest run.
    Tracks query ID, status, row counts, errors, and performance metrics.
    """
    QUEUED = 'queued'
    SUBMITTED = 'submitted'
    POLLING = 'polling'
    ...
    fee_type = models.CharField(
        default='FBA_FULFILLMENT_FEE', max_length=32,
    ...
    status = models.CharField(
        choices=STATUS_CHOICES, db_index=True, default=QUEUED, max_length=12
    )
    ...
    rows_received = models.IntegerField(
        default=0,
    ...
    rows_filtered_amzn = models.IntegerField(
        default=0,
        help_text='K1: amzn.* MSKUs excluded.'
    )
    rows_marked_adjustment = models.IntegerField(
        default=0,
        help_text='K2: quantity=0 or amount<0 marked as adjustment.'
    )
    rows_skipped_empty = models.IntegerField(
        default=0,
    ...
    class Meta:
        ...
        indexes = [
            models.Index(fields=['marketplace', '-submitted_at'], name='ix_dkil_mp_date'),
            models.Index(fields=['status', '-submitted_at'], name='ix_dkil_status_date'),
        ]
