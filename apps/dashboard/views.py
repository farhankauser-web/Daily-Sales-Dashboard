...
def api_sync_pnl_month(request):
    """
    Build one month's P&L from the Reports Repository — the Date Range
    Transaction report, which is what Amazon's own statement is built from.
    JSON: {marketplace, month:'YYYY-MM'}.

    WHAT CHANGED, AND WHY
        This called rebuild_settlement_month, which assembled the month from
        Settlement Flat File V2. Checked against Amazon's own Date Range
        Summary statement for USA July 2026, settlement was wrong on every
        order-level line:

            gross_sales   settlement 1,361,983.70   Amazon 1,114,203.14
            commission    settlement   197,392.88   Amazon   159,277.47
            promo         settlement     2,425.60   Amazon    22,117.54

        Account-level charges (advertising, FBA fees, AWD) were accurate to
        under 1%. Only the order lines were wrong — settlement yields ~20%
        more order rows than Amazon attributes to the month.

        The repository report reconciles EXACTLY: contribution before COGS of
        324,440.90 against Amazon's 324,440.90, delta -0.00. The parser for it
        already existed (unified_txn_importer); the settlement rebuild was
        overwriting its output.

    THE CURRENT MONTH
        The report is posted-date based and includes deferred transactions, so
        a month is complete on the 1st of the following month rather than
        waiting for settlement cycles that close between the 3rd and the 12th.
        A month whose report Amazon has not generated yet returns a clear
        "not available" rather than a partial figure — a half-built P&L is
    ...
    return JsonResponse({
        'status': 'ok',
        'message': (
            f'{month:%B %Y} built from the Reports Repository — '
            f'gross sales {float(row.amount):,.2f} across {row.units:,} units.'
            if row else
            f'{month:%B %Y} — no repository report available for this month '
            f'yet. Amazon generates it from the 1st of the following month.'),
